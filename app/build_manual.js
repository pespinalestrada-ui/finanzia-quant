...
{ titulo:"Bloque 1 · Para el día a día (empieza por aquí)", tabs:[
  { n:"📊 Factores", nv:"Básico",
    ...
    ve:["La de arriba (rank 1) es la 'mejor nota' del grupo.","La columna 'señal' te dice: COMPRAR (las mejores), EVITAR (las peores) o neutral.","Los números z son solo la posición dentro del grupo: + es bueno, − es malo.","**z_quality** = lo sólida que es la empresa. Dentro de esa nota mandan el **ROIC** (40%), el ROE (25%), el margen (20%) y la poca deuda (15%).","La columna **ROIC %** es lo que renta el capital que de verdad trabaja, mediana de varios años. Por encima de ~10% crea valor; por debajo lo destruye. Sale vacía en bancos, donde no aplica.","Puede pasar que una empresa con ROIC malo salga arriba: el ranking pesa también lo barata que está y su empuje. Mira la nota Y el ROIC, no solo el puesto."],
  ...
  { n:"3 · Screener", nv:"Básico",
    ...
    ve:["Ordenadas por fuerza/empuje.","La columna **ROIC** y **Calidad** te dicen si además el NEGOCIO gana dinero: 'crea valor', 'ok' o 'destruye'. Vacío en bancos.","El Score **no** incluye el ROIC a propósito: mide empuje, no calidad. Sirve para cazar el caso 'sube mucho pero la empresa no gana dinero'."],
...
{ titulo:"Bloque 2 · Practicar y operar (sin dinero real)", tabs:[
  ...
  { n:"🪙 Veredicto Cripto", nv:"Intermedio",
    ...
    ve:["Igual que el Veredicto normal: COMPRAR / MANTENER / VENDER.","Incluye el 'miedo y codicia' del mundo cripto.","Aquí **no** verás el motivo de calidad por ROIC: una cripto no tiene cuentas anuales, así que no hay ROIC que calcular. No es un olvido.","En su lugar está **Calidad de la red**, que mide dos cosas que sí existen: cuánto **volumen** mueve al día (si es poco, no podrás salir sin hundir el precio) y cuánto queda **por emitir** (lo que falta te diluye, como una empresa que no para de sacar acciones).","Ojo: eso mide la *fontanería*, no si la moneda vale algo. Una memecoin muy negociada puntúa bien aquí y sigue siendo una memecoin."],
...
{ titulo:"Bloque 3 · El sistema automático (avanzado)", tabs:[
  ...
  { n:"🎯 Marcador", nv:"Básico",
    q:"La pantalla que dice si el sistema acierta DE VERDAD. Cada Veredicto y cada Forecast quedan apuntados ANTES de saber qué pasa, y luego se comprueba a 7, 30 y 90 días si se cumplieron.",
    u:"Elige 'En vivo' (tus consultas) o 'Reconstruido' (llamadas históricas) y pulsa Ver marcador. El de en vivo se llena solo según vayas usando el panel.",
    ve:["Dos barras por plazo: la gris es lo que hace el activo **por sí solo**, la violeta lo que acierta el sistema. **Si son iguales, el sistema no aporta nada.**","Lo que importa es la diferencia en puntos que sale encima, no el porcentaje de acierto. Acertar el 54% en algo que sube el 54% de las veces es habilidad CERO.","Con menos de 30 llamadas no da porcentaje: dice 'muestra insuficiente'. Un 70% sobre 7 llamadas es ruido puro.","El 'Reconstruido' es solo el núcleo técnico (medias, RSI, MACD...), NO el Veredicto completo: los fundamentales no se pueden reconstruir sin mirar al futuro."],
    hacer:"Es el juez de todo lo demás. Si dice que no hay ventaja, hazle caso: úsalo para operar MENOS, no para buscar otra configuración. Ojo al aviso del periodo: en un tramo muy alcista, cualquier sistema que se salga pierde contra estar dentro." },
  { n:"🧪 CPCV", nv:"Avanzado",
...
{ titulo:"Bloque 4 · Matemática de fondo (solo si te pica la curiosidad)", tabs:[
  ...
  { n:"🌐 Superficie", nv:"Avanzado",
    q:"Qué precio le pone el mercado al miedo. Si la teoría fuera cierta, todas las opciones de una misma empresa tendrían la misma volatilidad. No la tienen, y en esa deformación hay información.",
    u:"Escribe un activo de EEUU (SPY, AAPL, QQQ...) y pulsa Dibujar superficie. Los activos europeos no tienen cadena de opciones en la fuente de datos.",
    ve:["**La sonrisa** (izquierda): una curva por vencimiento. Sube mucho hacia la izquierda porque **protegerse siempre cuesta más** que apostar a la subida.","**El sesgo** en puntos: cuánto más caro es cubrir una caída del 10% que apostar a una subida del 10%. En bolsa siempre es positivo.","**Estructura temporal** (derecha): lo normal es que la volatilidad suba con el plazo. Si se INVIERTE (corto por encima de largo), el mercado está descontando un susto inmediato.","La línea gris es la volatilidad **realizada**: lo que el activo se ha movido de verdad. Si la implícita está por encima, vender opciones cobra prima; si está por debajo, la protección está barata."],
    hacer:"Míralo antes de comprar o vender opciones, y junto a 🛡️ Cobertura: el sesgo que sale aquí es exactamente lo que encarece la put protectora. Si la estructura se invierte, el mercado sabe algo a corto plazo." },
  { n:"🛡️ Cobertura", nv:"Intermedio",
    q:"Poner un SUELO a lo que puedes perder en una posición, que es lo que hace una aerolínea cuando compra el queroseno por adelantado. Te dice cuánto cuesta el seguro y si compensa.",
    u:"Escribe el activo, cuántas acciones tienes, cuánta caída aceptas (el suelo) y a cuántos días. Pulsa Calcular cobertura.",
    ve:["**Put protectora**: pagas una prima y a cambio no pierdes más del suelo. El techo de ganancia sigue intacto. Es un seguro.","**Collar**: la misma put, pagada vendiendo una call. Casi gratis, pero renuncias a la subida por encima del techo. Es lo que hacen las empresas de verdad.","**Llevar menos**: sin opciones ni primas, simplemente invertir una parte. Sale el porcentaje que te deja la misma pérdida máxima en la peor caída que ese activo ha tenido de verdad.","La gráfica es el resultado según dónde acabe el precio: fíjate en cómo la línea sin cubrir se va al fondo mientras las cubiertas se aplanan."],
    hacer:"Mira el **coste anualizado** de la put. Si sale por encima del 6-8%, protegerte todo el año se come la rentabilidad que esperas de la bolsa: el seguro sale más caro que el riesgo. Cubrirse tiene sentido cuando NO PUEDES permitirte la caída (tienes que vender pronto), no cuando crees que va a pasar." },
  { n:"🔔 Colas", nv:"Intermedio",
    q:"Cuánto miente la campana de Gauss con tus activos. Casi toda la estadística financiera de manual supone que los días extremos son rarísimos. No lo son.",
    u:"Escribe hasta 3 activos y pulsa 'Medir colas'.",
    ve:["La **curtosis en exceso** mide el grosor de las colas: una campana normal vale 0. El SPY vale casi 15.","La gráfica está en escala logarítmica a propósito: es la única forma de ver las colas. Fíjate en cómo la línea dorada (la campana) se desploma a cero mientras las barras siguen hasta ±10σ.","**Días de más de 4σ**: el SPY tuvo 38 en 20 años; la campana predecía 0,32. Es 119 veces más.","El **peor día** en sigmas, y cada cuánto tocaría si el mundo fuera normal. Suelen salir cifras absurdas: más años que la edad del universo."],
    hacer:"Es el porqué de media suite: el riesgo de cola se mide en 📉 EVT y no con la normal, el Sharpe lleva corrección por curtosis, y la resta teórica de 📉 Deriva vol. sale exagerada justo por esto. Si alguien te vende un riesgo calculado con la campana, ya sabes cuánto se queda corto." },
  { n:"📉 Deriva", nv:"Intermedio",
    q:"Cuánta rentabilidad se te come la volatilidad cada año. La rentabilidad que se publica es la MEDIA; la que de verdad acumulas es más baja, y la diferencia crece muy rápido con lo que se mueve el activo.",
    u:"Escribe varios activos y pulsa Medir. El desplegable 'Qué medir' tiene tres modos: lo que se come la volatilidad, hasta dónde compensa apalancar, y qué te toca a TI (no a la media).",
    ve:["Dos barras por activo: la **media anual** (lo que se publica) y el **CAGR real** (lo que te llevas). Encima, los puntos que se pierden.","SPY se deja 0,6 puntos al año; TQQQ se deja **15,9**. La diferencia entre ambos no es la rentabilidad: es la volatilidad.","La curva de apalancamiento tiene forma de campana: hay un punto óptimo y, pasado otro, apalancar te deja PEOR que no apalancar.","**¿Qué me toca a MÍ?**: simula miles de inversores con el mismo activo y las mismas reglas. Solo ~4 de cada 10 llegan a la rentabilidad 'media' que se anuncia; al de en medio le toca menos. La media la levantan unos pocos caminos afortunados, y tú vives uno solo."],
    hacer:"Es la razón matemática de que 🛞 Vol objetivo funcione: bajar la volatilidad no solo quita sustos, sube lo que acumulas. Y explica por qué un ETF x3 puede perder a largo plazo aunque el índice suba. Ojo: los apalancamientos que salen son OPTIMISTAS (no cuentan intereses ni comisiones)." },
  { n:"⏳ OU", nv:"Avanzado",
