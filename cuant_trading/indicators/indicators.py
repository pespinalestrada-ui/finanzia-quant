...
def hurst(ts, max_lag=20):
    """Exponente de Hurst. ~0.5 paseo aleatorio, >0.55 tendencia, <0.45 reversión."""
    lags = range(2, max_lag)
    lags = [lag for lag in lags if lag < len(ts)]
    if len(lags) < 3:
        return 0.5
...
def calcular_todos(df):
    ...
    df["OBV"] = obv(df)
    df["MFI"] = mfi(df)
    df["HURST"] = c.rolling(100).apply(lambda x: hurst(x), raw=True)
    return df
...
def señales_dict(df):
    ...
    h_val = last.get("HURST", float("nan"))
    if not np.isnan(h_val):
        reg = "tendencia" if h_val > 0.55 else "reversión" if h_val < 0.45 else "paseo aleatorio (rango)"
        out.append(("Hurst (100d)", f"{h_val:.2f}", reg))
    return out
