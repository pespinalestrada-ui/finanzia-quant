...
# shim: forecast_tool importa "src.data_loader"; en el Space vive plano en modules/
import types, importlib
_dl = importlib.import_module("data_loader")
_src_pkg = types.ModuleType("src"); _src_pkg.data_loader = _dl
sys.modules["src"] = _src_pkg
sys.modules["src.data_loader"] = _dl

# motores pesados opcionales (no instalados en el Space gratuito)
    ...
    with gr.Blocks(title="FinanzIA — Mesa cuantitativa", head=_head) as app:
        ...
        with gr.Tab("1 · Forecast"):
            with gr.Row():
                ...
                _motores = (["Prophet (rápido)", "LSTM (red neuronal)"]
                            + (["NeuralProphet (AR-Net)"] if NEURALPROPHET_OK else [])
